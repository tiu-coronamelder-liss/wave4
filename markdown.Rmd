...
•	Maatschappelijk-gerelateerde overtuigingen(*α* = .83): de overtuiging dat de CoronaMelder bijdraagt aan Nederlandse economie en aan goed burgerschap, het vertrouwen dat men heeft in de overheid, de overtuiging dat risicogroepen worden beschermd, en dataveiligheid 
•	Privacy-gerelateerde overtuigingen (*α* = .74): de mate waarin men de overtuiging heeft dat de CoronaMelder de locatie bijhoudt en persoonsgegevens opslaat
•	Angst-gerelateerde overtuigingen (*α* = .66): de mate waarin men bang is voor de CoronaMelder, en de mate waarin men bang is voor notificaties van de CoronaMelder
•	Overtuigingen over complottheorieën (*α* = .71): de mate waarin men de overtuiging heeft dat het coronavirus in een laboratorium is gemaakt, en de overtuiging dat de uitbraak van het coronavirus te maken heeft met de aanleg van 5G.
...
```{r Model fit table SEM, echo = FALSE, tab.cap ="Model fit van de Structural Equation Models"}

#Model fit SEMS
dftab <- data.frame(Model = c("UTAUT", "HBM", "Adherence no symptoms", "Adherence symptoms"), 'Chi-Squared' = c('105.428', '71.648', '250.507', '250.609'), CFI = c('.999', '.993', '.949', '.950'), RMSEA = c('.044', '.054', '.071', '.071'), '90percent-CI' = c('[.035, .054]', '[.042, .067]', '[.061, .081]', '[.061, .081]'), SRMR = c('.029', '.069', '.052', '.052'), 'R-squared' = c('.923', '.487', '.084', '.204'))
...
```{r Model fit table log reg, echo = FALSE, tab.cap="Model fit van de logistische regressie"}
