...
Uit de analyse bleek dat verwachte inspanning (*β* = .23, *p* = .002), verwachte effectiviteit (*β* = .20, *p* < .001) en sociale invloed (*β * = .38, *p* < .001) positief samenhangen met CoronaMelder adoptie. Ook het gevoel van verplichting tot gebruik (d.w.z., hoe sterker het gevoel van verplichting, hoe hoger de CoronaMelder adoptie; *β* = .21, *p* < .001), en faciliterende omstandigheden (*β* = .20, *p* = .017) hangen positief samen met CoronaMelder gebruik.   
...
Uit de analyse bleek dat gepercipieerde voordelen een positieve (*β* = .47, *p* < .001), en gepercipieerde barrières een negatieve samenhang hebben (*β* = -.14, *p* < .001) met CoronaMelder gebruik. Daarnaast heeft de mate waarin men het gevoel heeft in staat te zijn om de CoronaMelder te gebruiken (zelfeffectiviteit), een positieve relatie met CoronaMelder gebruik (*β* = .31, *p* < .001). De perceptie van de vatbaarheid voor het coronavirus, de ernst van een besmetting, en de de mate waarin men de CoronaMelder voorbij heeft zien komen in de media/het nieuws hingen in dit model niet significant samen met CoronaMelder gebruik. 
...
Uit de logistische regressie analyse bleek dat maatschappelijke overtuigingen positief samenhingen met CoronaMelder gebruik (*b* = 1.29, *p* < .001). Ook de adequaatheid van de techniek hing positief samen met CoronaMelder gebruik (*b* = 0.17, *p* = .030). Overtuigingen over privacy-gerelateerde kwesties hebben een negatieve samenhang met CoronaMelder gebruik (*b* = -.12, *p* = .003), evenals angst-gerelateerde overtuigingen (*b* = -.30, *p* < .001). Overtuigingen over complottheorieën, de kans om anderen te besmetten als men zelf besmet is, en de ernst van het besmetten van anderen als men zelf  besmet is, bleken geen significante samenhang met CoronaMelder gebruik te hebben.  
...
Om de intentie tot het opvolgen van de in de melding gegeven adviezen te verklaren zijn twee voorspellende modellen getest: een voor de intentie tot opvolging in het hypothetisch geval van klachten en een voor de intentie tot opvolging in het hypothetisch geval van afwezigheid van klachten. Een overzicht van de model fit parameters is te vinden in de Appendix.
...
Zelfeffectiviteit hangt positief samen met de intentie tot naleving van de adviezen (*β* = .21, *p* < .001, *α* = .88), waarbij een hogere zelfeffectiviteit leidt tot een sterkere intentie tot naleving van de adviezen. Ook de mate waarin men zichzelf ontvankelijk acht voor besmetting met het coronavirus hing significant positief samen met de intentie tot opvolging van de adviezen (*β* = .176, *p* = .002, *α* = .78). De overige variabelen (d.w.z., gepercipieerde voordelen, *α* = .91, gepercipieerde barrières, *α* = .85, en gepercipieerde ernst van een besmetting met het coronavirus, *α* = .72) hadden geen significante samenhang met de intentie tot het naleven van de adviezen.
 	
*Situatie met symptomen.* Net als in de situatie zonder symptomen werd aan participanten gevraagd in hoeverre zij de intentie hadden om de adviezen uit de app op te volgen na het zien van een melding van de CoronaMelder, maar dit keer in een situatie met symptomen (*α* = .87). Wederom hing zelfeffectiviteit positief samen met de intentie tot opvolgen van de adviezen (*β* = .41, *p* < .001). Daarnaast hing gepercipieerde ernst significant positief samen met de intentie tot opvolging van de adviezen (*β* = .14, *p* = .017). De overige variabelen (d.w.z., gepercipieerde voordelen, gepercipieerde barrières en gepercipieerde ontvankelijkheid voor een besmetting met het coronavirus) hadden geen significante samenhang met de intentie tot het opvolgen vn de adviezen. 
...
## Tabellen verklarende modellen - predictie analyses 

Hieronder zijn de tabellen van de verklarende modellen te zien met de op de theorie gebaseerde predictoren (exclusief controlevariabelen). 
```{r include = FALSE}
##### Load packages
# For importing SPSS files
library(foreign)
...
```{r echo = FALSE, tab.cap="Verklarend model op basis van UTAUT variabelen met CoronaMelder gebruik als afhankelijke variabele"}
...
```{r echo = FALSE, tab.cap="Verklarend model op basis van het HBM met CoronaMelder gebruik als afhankelijke variabele"}
data_table <- w4_hbm[,-1]
# Build the basic table
ft <- flextable(
  head(data_table, n = nrow(data_table))
...
```{r echo = FALSE, tab.cap="Verklarend model intentie tot naleving gedragsmaatregelen, met symptomen"}
...
```{r echo = FALSE, tab.cap="Verklarend model intentie tot naleving gedragsmaatregelen, zonder symptomen"}
