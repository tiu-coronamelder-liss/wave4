...
- Het kost(te) mij veel tijd en energie om de CoronaMelder app te gebruiken
- Ik denk dat het mij veel tijd en energie kost om de CoronaMelder app te gebruiken
...
Uit de SEM analyse bleek dat verwachte inspanning (*β* = .23, *p* = .002), verwachte effectiviteit (*β* = .20, *p* < .001) en sociale invloed (*β * = .38, *p* < .001) positief samenhangen met CoronaMelder gebruik. Ook het gevoel van verplichting tot gebruik (d.w.z., hoe sterker het gevoel van verplichting, hoe meer waarschijnlijk dat iemand een CoronaMelder gebruiker is; *β* = .21, *p* < .001), en faciliterende omstandigheden (*β* = .20, *p* = .017) hangen positief samen met CoronaMelder gebruik.   
...
Uit de logistische regressie analyse bleek dat maatschappelijke overtuigingen positief samenhingen met CoronaMelder gebruik (*b* = 1.29, *p* < .001). Ook de adequaatheid van de techniek hing positief samen met CoronaMelder gebruik (*b* = 0.12, *p* = .030). Overtuigingen over privacy-gerelateerde kwesties hebben een negatieve samenhang met CoronaMelder gebruik (*b* = -.12, *p* = .003), evenals angst-gerelateerde overtuigingen (*b* = -.30, *p* < .001). Overtuigingen over complottheorieën, de kans om anderen te besmetten als men zelf besmet is, en de ernst van het besmetten van anderen als men zelf  besmet is, bleken geen significante samenhang met CoronaMelder gebruik te hebben.  
...
Zelfeffectiviteit hangt positief samen met de intentie tot naleving van de adviezen (*β* = .21, *p* < .001, *α* = .88). Ook de mate waarin men zichzelf ontvankelijk acht voor besmetting met het coronavirus hing significant positief samen met de intentie tot opvolging van de adviezen (*β* = .176, *p* = .002, *α* = .78). De overige variabelen (d.w.z., gepercipieerde voordelen, *α* = .91, gepercipieerde barrières, *α* = .85, en verwachte ernst van een besmetting met het coronavirus, *α* = .72) hadden geen significante samenhang met de intentie tot het naleven van de adviezen.
 	
*Situatie met symptomen.* Net als in de situatie zonder symptomen werd aan participanten gevraagd in hoeverre zij de intentie hadden om de adviezen uit de app op te volgen na het zien van een melding van de CoronaMelder, maar dit keer in een hypothetische situatie met symptomen. Dezelfde drie gedragsadviezen als hierboven werden gecombineerd tot de afhankelijke variabele *intentie tot opvolgen adviezen met symptomen* (*α* = .87). Wederom hing zelfeffectiviteit positief samen met de intentie tot opvolgen van de adviezen (*β* = .41, *p* < .001). Daarnaast hing de verwachte ernst van een besmetting significant positief samen met de intentie tot opvolging van de adviezen (*β* = .14, *p* = .017). De overige variabelen (d.w.z., gepercipieerde voordelen, *α* = .91, gepercipieerde barrières, *α* = .85, en verwachte vatbaarheid voor een besmetting met het coronavirus, *α* = .78) hadden geen significante samenhang met de intentie tot het opvolgen vn de adviezen. 
