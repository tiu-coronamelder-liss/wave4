...
**Intentie tot adherentie aan gedragsadviezen in de melding** 
...
Om te onderzoeken welke technologie adoptie variabelen samenhangen met CoronaMelder gebruik, is er een analyse uitgevoerd met de UTAUT voorspellers als onafhankelijke variabelen en CoronaMelder adoptie als afhankelijke variabele. Uit de analyse blijkt dat verwachte inspanning, verwachte effectiviteit en sociale invloed positief samenhangen met CoronaMelder adoptie. Ook het gevoel van verplichting tot gebruik hangt positief samen met CoronaMelder adoptie (d.w.z., hoe sterker het gevoel van verplichting, hoe hoger de CoronaMelder adoptie). 

### Verklarend model adoptie op basis van het HBM
Daarnaast is onderzocht in hoeverre variabelen gerelateerd aan interventies gericht op gezondheid een rol spelen in het voorspellen van CoronaMelder adoptie. Hiervoor is een analyse uitgevoerd met onafhankelijke variabelen zoals in het HBM gespecifieerd, en CoronaMelder gebruik als afhankelijke variabele. Uit de analyse bleek dat gepercipieerde voordelen een positieve, en gepercipieerde nadelen een negatieve invloed hebben op CoronaMelder adoptie. Daarnaast heeft de mate waarin men het gevoel heeft in staat te zijn om de CoronaMelder te gebruiken (m.a.w., self-efficacy), een positieve realtie met CoronaMelder gebruik. De perceptie van de vatbaarheid om besmet te raken met het coronavirus (perceived susceptibilit) en hoe erg men het zou vinden om besmet te raken (perceveived severity) bleken geen rol te spelen, evenals leeftijd en geslacht. 

### Verklarend model adoptie op basis van contextspecifieke variabelen
Ook variabelen specifiek voor de context van de huidige corona pandemie zijn onderzocht. In deze analyse is bekeken in hoeverre deze context-specifieke variabelen samenhangen met CoronaMelder gebruik. Hieruit bleek dat angst voor de CoronaMelder en privacy overwegingen (d.w.z., mate waarin men denkt dat persoonlijke gegevens worden opgeslagen) negatief samen hangen met CoronaMelder gebruik. De perceptie van ‘goed burgerschap’ wanneer men de CoronaMelder installeert, een positief effect van de CoronaMelder op de Nederlandse economie, en de potentie tot bescherming van de risicogroepen zijn redenen die positief samenhangen met CoronaMelder adoptie.

TODO voor alledrie de modellen: insert stats en afbeelding model. 
...
TABEL Notification_behavior_quarantaine HIER. 
