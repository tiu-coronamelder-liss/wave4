...
```{r Regression model UTAUT DUMMIES}
...
```{r Regression model HBM DUMMIES}
...
#Logistic regression
mylogit <- glm(Behavior_UTAUT_r ~ Beliefs_society + Beliefs_fears + Beliefs_monitoring + Beliefs_conspiracy + HBM_PSus_other2 + HBM_PSev_other2 + Beliefs_technologyperformance + lftdcat + geslacht, data = data_wave4, family=binomial)
...
# Compare model with variabels to model with only the intercept
...
```{r Regression model context-specific variables WITHOUT FEAR}
...
#Context_fears =~ Beliefs_fear + Beliefs_notificationfear

#Regression model
Behavior_UTAUT_r ~ Context_society + Context_monitoring + Context_conspiracy + lftdcat + geslacht + Beliefs_technologyperformance_resc + HBM_PSev_other2 + HBM_PSus_other2

#Covariances between independent vars
Context_society ~~ Context_monitoring + Context_conspiracy + lftdcat + geslacht + Beliefs_technologyperformance_resc + HBM_PSev_other2 + HBM_PSus_other2
Context_monitoring ~~ Context_conspiracy + lftdcat + geslacht + Beliefs_technologyperformance_resc + HBM_PSev_other2 + HBM_PSus_other2
Context_conspiracy ~~ lftdcat + geslacht + Beliefs_technologyperformance_resc + HBM_PSev_other2 + HBM_PSus_other2
lftdcat ~~ geslacht + Beliefs_technologyperformance_resc + HBM_PSev_other2 + HBM_PSus_other2
geslacht ~~ Beliefs_technologyperformance_resc + HBM_PSev_other2 + HBM_PSus_other2
Beliefs_technologyperformance_resc ~~ HBM_PSev_other2 + HBM_PSus_other2
HBM_PSev_other2 ~~ HBM_PSus_other2'


# Comparing different models http://www.talkstats.com/threads/sem-comparing-models-with-different-variables.38047/ 

fit_Context1 <- sem(Context_w4, data = data_wave4, ordered = TRUE, estimator = "WLSMV", se = 'bootstrap', test = 'bootstrap') #Ordered = TRUE tells us that all the endogenous variables are categorical. #In this case, this is the Behavior_UTAUT var. 
