...
```{r Preparation}
#List of recodings that must be performed before runnng the analysis. Some of the beliefs are recoded into dummy variables.
...
# Some of the 99's ('I don't know') should be NA for the factor analysis
...
# Some variables must be rescaled (the ones measured on a 1 to 4 scale) for the factor analysis
data_wave4$Beliefs_identitymonitoring_resc <- scales:::rescale(data_wave4$Beliefs_identitymonitoring, to = c(1, 7)) 
#View(data_wave4[,c("Beliefs_identitymonitoring", "Beliefs_identitymonitoring_resc")])
data_wave4$Beliefs_locationmonitoring_resc <- scales:::rescale(data_wave4$Beliefs_locationmonitoring, to = c(1, 7)) 
#View(data_wave4[,c("Beliefs_locationmonitoring", "Beliefs_locationmonitoring_resc")])
data_wave4$Beliefs_datasafety_resc <- scales:::rescale(data_wave4$Beliefs_datasafety, to = c(1, 7)) 
#View(data_wave4[,c("Beliefs_datasafety", "Beliefs_datasafety_resc")])
data_wave4$Beliefs_Conspiracy1_resc <- scales:::rescale(data_wave4$Beliefs_Conspiracy1, to = c(1, 7)) 
#View(data_wave4[,c("Beliefs_Conspiracy1", "Beliefs_Conspiracy1_resc")])
data_wave4$Beliefs_Conspiracy2_resc <- scales:::rescale(data_wave4$Beliefs_Conspiracy2, to = c(1, 7)) 
#View(data_wave4[,c("Beliefs_Conspiracy2", "Beliefs_Conspiracy2_resc")])
data_wave4$Beliefs_technologyperformance_resc <- scales:::rescale(data_wave4$Beliefs_technologyperformance, to = c(1, 7)) 
...
Context_fears =~ aa*Beliefs_fear + aa*Beliefs_notificationfear

#Regression model
Behavior_UTAUT_r ~ Context_society + Context_monitoring + Context_conspiracy + lftdcat + geslacht + Beliefs_technologyperformance_dummy + HBM_PSevother_dummy + HBM_PSusother_dummy + Context_fears

#Covariances between independent vars, see https://www.youtube.com/watch?v=n-ULF6BGVw0 

Context_society ~~ Context_monitoring + Context_conspiracy + lftdcat + geslacht + Beliefs_technologyperformance_dummy + HBM_PSevother_dummy + HBM_PSusother_dummy + Context_fears

Context_monitoring ~~ Context_conspiracy + lftdcat + geslacht + Beliefs_technologyperformance_dummy + HBM_PSevother_dummy + HBM_PSusother_dummy + Context_fears

Context_conspiracy ~~ lftdcat + geslacht + Beliefs_technologyperformance_dummy + HBM_PSevother_dummy + HBM_PSusother_dummy + Context_fears

lftdcat ~~ geslacht + Beliefs_technologyperformance_dummy + HBM_PSevother_dummy + HBM_PSusother_dummy + Context_fears

geslacht ~~ Beliefs_technologyperformance_dummy + HBM_PSevother_dummy + HBM_PSusother_dummy + Context_fears

Beliefs_technologyperformance_dummy ~~ HBM_PSevother_dummy + HBM_PSusother_dummy + Context_fears

HBM_PSevother_dummy ~~ HBM_PSusother_dummy + Context_fears

HBM_PSusother_dummy ~~ Context_fears'
...
# Create results table
w4_hbm <- standardizedsolution(fit_Context1) %>% filter(lhs == 'Behavior_UTAUT_r' & op == '~') %>% select(rhs, est.std, pvalue)
names(w4_hbm)[names(w4_hbm) == "rhs"] <- "variable"
w4_hbm = join(w4_hbm, var_lookup)
